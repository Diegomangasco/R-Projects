...
In alternativa è possibile utilizzare il pulsante in alto nel pannello dell'editor: il comando esegue tutti i *chunk*, inserisce il risultato nel documento e genera il file HTML.
...
sort(murders$population) #aggiungo TRUE per il descescente
```
...
# TODO: interire il codice necesario
or <- order( - murders$population)
murders[or, c("state", "population")]
```
...
# TODO: interire il codice necesario
celsius <- (5/9) * (city_temps$temperature_F -32)
city_temps$temperature_C <- celsius
city_temps
```
...
celsius <- (5/9) * (city_temps$temperature_F -32)
city_temps$temperature_C <- celsius
c <- round(city_temps$temperature_C, 3)
city_temps$temperature_C <- c
city_temps
```
...
```{r tasso omicidi}
# TODO: interire il codice necesario
murders$rate <- round(((murders$total*100000)/murders$population), 2)
murders
```
...
# TODO: interire il codice necesario
minus <- which( murders$rate <= five[2] )
print(length(minus))
```
...
```{r esercizio 3 - 3.3}
# TODO: interire il codice necesario
minew <- which( murders$region == "West", TRUE )
minus <- which( murders$rate <= five[2], TRUE )
common <- 1
t <- 1
for( i in 1:length(minew) ){
...
```
```{r}
minew2 <- murders[which(murders$region == "West" & murders$rate <= five[2], TRUE),]
minew2
```
...
# TODO: interire il codice necesario
states <- murders$abb %in% c("MA", "ME", "MI", "MO", "MU")
length(murders$state[states])
```
...
# TODO: interire il codice necessario
successione_Fibonacci <- function(n, fib = NA){
  if(all(is.na(fib))){
    fib <- 1:n
  }
  if(n == 1 | n == 2){
  ...
  fib <- successione_Fibonacci(n-1, fib)
  fib <- successione_Fibonacci(n-2, fib)
...
```{r scatter formula }
# TODO: interire il codice necesario
plot(population ~ total, data = murders, type = "l", col = "red")
```
...
```{r scatter log}
# TODO: interire il codice necesario
#plot(log(murders$population, base = exp(100)), murders$total)
plot(murders$population, murders$total, log="x")
```
...
murders
reor <- reorder(murders$region, murders$population, sum)
reor
```
...
# TODO: interire il codice necesario
reor <- reorder(murders$region, murders$rate, mean)
reor
boxplot(murders$rate ~ reor)
```
...
# TODO: interire il codice necesario
barplot(rate ~ state, data = murders, horiz = TRUE, las = 1)
```
...
# TODO: interire il codice necesario
reor2 <- reorder(murders$state, - murders$rate, sum)
reor2
barplot(murders$rate ~ reor2, horiz = TRUE, las = 1)
#finished
```