...
ui <- fluidPage(
    ...
    br(),
    fluidRow(
        column(12, offset = 1, fileInput("file", "Load csv file Marcatori Serie A", accept = c(".csv")))),
    br(),
    fluidRow(
        column(5, offset = 1, selectInput("select", "Seleziona i ruoli",
                choices = c("Tutti",
                            "Centrocampista",
                            "Attaccante",
                            "Difensore"), 
                selected = "Tutti",
                multiple = TRUE)),
        column(5, offset = 1, selectInput("select2", "Seleziona le squadre",
